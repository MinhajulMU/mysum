...
class Sentence(Sequence[Word]):
    ...
    def __str__(self) -> str:
        ...
        if isinstance(self.label, bool):
            ...
            label = self.label
        return '{}{}{}'.format(label, self.DELIMITER, words_str)
...
class Paragraph(Sequence[Sentence]):
    ...
    def words(self) -> List[Word]:
        punctuationss = '@#$%^&*();:!-_=+[]{\/}|"`<>?'
        return [ word.lower().replace(word.lower(),'UNK') if word.lower() in punctuationss else word.lower() for sent in self.sentences for word in sent]
...
class Document(Sequence[Paragraph]):
    ...
    def preprocess(self) -> None:
        ...
        if self.remove_puncts:
            self.paragraphs = [para.filter_words(lambda w: w not in punctuationss)
                               for para in self.paragraphs]
    ...
    def to_dict(self) -> dict:
        ...
        if self.summary is not None:
            res['summary'] = [summ.filter_words(lambda w: w not in punctuationss).lower()
                               for summ in self.summary]
